...
bool EzImageLoaderBMP::isALoadableFileExtension(const c8* fileName) const
{
...
void EzImageLoaderBMP::decompress8BitRLE(u8*& bmpData, s32 size, s32 width, s32 height, s32 pitch) const
{
	u8* p = bmpData;
...
void EzImageLoaderBMP::decompress4BitRLE(u8*& bmpData, s32 size, s32 width, s32 height, s32 pitch) const
{
...
EzImage* EzImageLoaderBMP::loadImage(EzFile* file) const
{
	...
	{
	case 1:
		image = new EzImage(ECF_A1R5G5B5, EzDimension2d<s32>(header.Width, header.Height));
		if (image)
			EzColorConverter::convert1BitTo16Bit(bmpData, (s16*)image->lock(), header.Width, header.Height, pitch, true);
		break;
	case 4:
		image = new EzImage(ECF_A1R5G5B5, EzDimension2d<s32>(header.Width, header.Height));
		if (image)
			EzColorConverter::convert4BitTo16Bit(bmpData, (s16*)image->lock(), header.Width, header.Height, paletteData, pitch, true);
		break;
	case 8:
		image = new EzImage(ECF_A1R5G5B5, EzDimension2d<s32>(header.Width, header.Height));
		if (image)
			EzColorConverter::convert8BitTo16Bit(bmpData, (s16*)image->lock(), header.Width, header.Height, paletteData, pitch, true);
		break;
	case 16:
		image = new EzImage(ECF_A1R5G5B5, EzDimension2d<s32>(header.Width, header.Height));
		if (image)
			EzColorConverter::convert16BitTo16Bit((s16*)bmpData, (s16*)image->lock(), header.Width, header.Height, pitch, true);
		break;
	case 24:
		image = new EzImage(ECF_R8G8B8, EzDimension2d<s32>(header.Width, header.Height));
		if (image)
			EzColorConverter::convert24BitTo24Bit(bmpData, (u8*)image->lock(), header.Width, header.Height, pitch, true, true);
		break;
	case 32: // thx to Reinhard Ostermeier
		image = new EzImage(ECF_A8R8G8B8, EzDimension2d<s32>(header.Width, header.Height));
		if (image)
			EzColorConverter::convert32BitTo32Bit((s32*)bmpData, (s32*)image->lock(), header.Width, header.Height, pitch, true);
		break;
	};
	if (image)
		image->unlock();
