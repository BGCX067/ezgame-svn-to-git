...
bool EzImageLoaderTGA::isALoadableFileExtension(const c8* fileName) const
{
...
u8 *EzImageLoaderTGA::loadCompressedImage(EzFile* file, const STGAHeader& header) const
{
	...
	case 16:
		{
			image = EzNew EzImage(ECF_A1R5G5B5,
				EzDimension2d<s32>(header.ImageWidth, header.ImageHeight));
			if (image)
				EzColorConverter::convert16BitTo16Bit((s16*)data,
	...
	case 32:
		{
			image = EzNew EzImage(ECF_A8R8G8B8,
				EzDimension2d<s32>(header.ImageWidth, header.ImageHeight));
			if (image)
				EzColorConverter::convert32BitTo32Bit((s32*)data,
