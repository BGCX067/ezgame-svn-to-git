...
EzD3DX9Texture::EzD3DX9Texture(EzD3DX9Driver* driver, const EzDimension2d<s32>& size, const char* name)
...
bool EzD3DX9Texture::createMipMaps(u32 level)
{
//	if (level==0)
//		return true;
//
...
bool EzD3DX9Texture::createTexture(u32 flags, EzImage * image)
{
	EzDimension2d<s32> optSize;
...
bool EzD3DX9Texture::copyTexture(EzImage * image)
{
	...
	if( Texture && image )
	{
		...
		{
			tmpImage = new EzImage( ECF_A8R8G8B8, image );
			deleteTmpImage = true;
		...
		if( !SurfaceHasSameSize )
		{
			...
			tmpImage = new EzImage( m_eColorFormat, m_TextureSize );
...
inline s32 EzD3DX9Texture::getTextureSizeFromSurfaceSize(s32 size) const
{
	s32 ts = 0x01;
...
EDRIVER_TYPE EzD3DX9Texture::getDriverType() const
{
	return EDT_DIRECTX9;
}
