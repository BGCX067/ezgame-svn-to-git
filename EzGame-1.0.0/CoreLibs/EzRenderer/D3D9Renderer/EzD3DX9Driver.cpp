  ...
  m_pGpuProgramManager(NULL), m_bTransformation3DChanged(true)
{
	...
	for(u32 n = 0; n < NUM_TEXTURE_SUPPORT; n++)
...
bool EzD3DX9Driver::beginScene(bool backBuffer, bool zBuffer, SColor color)
{
...
bool EzD3DX9Driver::setResize(const EzDimension2d<s32>& size, void* WindowId)
{
	//if (!m_pID3DDevice)
	//	return false;
...
bool EzD3DX9Driver::endScene()
{
	...
	//for(u32 ui = 0 ; ui < m_pEntity->getCount(); ++ui)
	//{
	//	EzGeometry* pGeometry = m_pEntity->getGeometry(ui);
	...
	//		for(u32 uiCount = 0 ; uiCount < pGeometry->getPropertySize(); ++uiCount)
	//		{
	//			setProperty(pGeometry->getAtProperty(uiCount));
	//		}

	//		EzDynamicEffectState* pEffectState = pGeometry->getEffectState();

	//		setShadingMode(ESHADE_GOURAUD);

	//		if(pEffectState)
	//		{
	//			setLighting(true);
	//			setEffectState(pEffectState);
	//		}
	//		else
	//		{
	//			setLighting(false);
	...
	//		if(pGeometry->getSkinInstance() && pGeometry->getSkinInstance()->getHWSkinPartition() && isHardwareSkinning())
	//		{
	//			EzGPUProgram* pVertexProgram = pPass->getVertexProgram();

	//			if(pVertexProgram)
	//			{
	//				EzGpuProgramParameters* pParams = pPass->getVertexProgramParameters();

	//				EzMatrix4 worldViewProj;
	//				worldViewProj = m_Matrices[ETS_PROJECTION];
	//				worldViewProj *= m_Matrices[ETS_VIEW];
	//				worldViewProj *= pGeometry->getWorldTransform().getMatrix();

	//				pParams->setShaderConstant(0, worldViewProj.getTransposed().pointer(), 4);
	//			}
	//		}
	//		else
	//		{
	//			if(m_bTransformation3DChanged)
	//			{
	...
	//			m_pID3DDevice->SetTransform(D3DTS_WORLD, (D3DMATRIX*)((void*)&pGeometry->getWorldTransform().getMatrix()));

	//		}

	//		pVertexProgram = pPass->getVertexProgram();

	//		if(pVertexProgram)
	//		{
	//			bindGpuProgram(pVertexProgram);
	//		}

	//		drawBuffer(pRendererData);
	//		
	//	} while (pPass && pPass->getNext());
	//
	//}
	...
	if (FAILED(hr))
	{
		log(0, "DIRECT3D9 end scene failed.");
		return false;
	}
...
//! Draws a 3d line.
void EzD3DX9Driver::draw3DLine(const EzPoint3& start,	const EzPoint3& end, SColor color)
{
...
bool EzD3DX9Driver::setMultiSample(EMULTISAMPLE_TYPE eMultiSampleType)
{
	...
	{
		log(EZ_WARN_NOT_SUPPORT_MULTISAMPLE, "Not Support this MultiSampleTyped.(%d)",(u32) eMultiSampleType);
		return false;
	}
...
bool EzD3DX9Driver::initDriver(const EzDimension2d<s32>& screenSize,	HWND hWnd, u32 uiBits, bool bFullScreen, bool bPureSoftware,
		bool bHighPrecisionFPU, bool bVsync, EMULTISAMPLE_TYPE eMultiSampleType, bool bMuiltView)
{
	...
	{
		s32 nProduct = HIWORD(dai.DriverVersion.HighPart);
		s32 nVersion = LOWORD(dai.DriverVersion.HighPart);
		s32 nSubVersion = HIWORD(dai.DriverVersion.LowPart);
		s32 nBuild = LOWORD(dai.DriverVersion.LowPart);

		log(EZ_DIRVER_INFORMATION, "%s %s %d.%d.%d.%d", dai.Description, dai.Driver, nProduct, nVersion,
			nSubVersion, nBuild);
		...
		switch(dai.VendorId)
		{
			...
			//default: m_vendorName.format("%d Unknown VendorId: ",dai.VendorId);	break;
		}
	}
	...
	if (FAILED(hr))
	{
		log(EZ_ERROR_GET_ADAPTER_DISPLAY_MODE, "Error: Could not get Adapter Display mode.");
		return false;
	}
	...
	for(u32 ui = (u32) eMultiSampleType; ui > 0; --ui)
	{
	...
	if (m_bStencilBuffer)
	{
			...
			D3DRTYPE_SURFACE, m_D3DPresent.AutoDepthStencilFormat)))
		{
				...
				D3DRTYPE_SURFACE, m_D3DPresent.AutoDepthStencilFormat)))
			{
				...
				if(FAILED(m_pID3D->CheckDeviceFormat(m_uiAdapter, m_devtype,
					...
					D3DRTYPE_SURFACE, m_D3DPresent.AutoDepthStencilFormat)))
				{
					log(0, "Device does not support stencilbuffer, disabling stencil buffer.");
		...
		{
			log(0, "Depth-stencil format is not compatible with display format, disabling stencil buffer.");
			m_bStencilBuffer = false;
		}
	}
			...
			D3DRTYPE_SURFACE, m_D3DPresent.AutoDepthStencilFormat)))
		{
				...
				D3DRTYPE_SURFACE, m_D3DPresent.AutoDepthStencilFormat)))
			{
				...
				if(FAILED(m_pID3D->CheckDeviceFormat(m_uiAdapter, m_devtype,
					...
					D3DRTYPE_SURFACE, m_D3DPresent.AutoDepthStencilFormat)))
				{
					log(0, "Device does not support required depth buffer.");
					return false;
				}
			}
		}
	...
	if (bPureSoftware)
	{
		...
		if (FAILED(hr))
		{
			log(EZ_ERROR_CREATE_FAIL_D3D, "Was not able to create Direct3D9 device.");
			return false;
		}
	}
	else
	...
	{
		log(0, "Device not able to use stencil buffer, disabling stencil buffer.");
		m_bStencilBuffer = false;
	}
...
//	setRenderStates3DMode();
	...
	m_pBufferManager = EzNew EzD3DX9BufferManager(m_pID3DDevice);

	// creat GpuProgram Manager 
	m_pGpuProgramManager = EzNew EzD3DX9GPUProgramManager(m_pID3DDevice);
	...
	m_pRenderer = EzNew EzRenderer(this);

	EzRenderer::initializePools();
...
bool EzD3DX9Driver::setTexture(u32 eMap, EzTextureDesc* pTexDesc)
{
	EzTexture* source = pTexDesc->source;

	if (source && source->getDriverType() != EDT_DIRECTX9)
	{
		log(EZ_ERROR_TEXTURE_IS_NOT_BYD3D,"Tried to set a texture not owned by this driver.");
		return false;
	}

//	m_pCurrentTexture[stage] = pTexture;
	...
	{
		m_pID3DDevice->SetTexture(eMap, ((const EzD3DX9Texture*)source)->getDX9Texture());
	}
	return true;
}
...
void EzD3DX9Driver::drawVertexPrimitive(EPRIMITIVE_TYPE eType, u32 uiVertexStart, u32 uiIndexNum, u32 uiIndexStart)
{
	...
	case EPT_POINTS:
		...
		primCount = (u32) uiIndexNum;
		break;

	case EPT_LINES:
		primType = D3DPT_LINELIST;
		primCount = (u32) uiIndexNum / 2;
		break;

	case EPT_LINE_STRIP:
		primType = D3DPT_LINESTRIP;
		primCount = (u32) uiIndexNum - 1;
		break;

	case EPT_TRIANGLE_LIST:
		primType = D3DPT_TRIANGLELIST;
		primCount = (u32) uiIndexNum / 3;
		break;
	...
	u32& uiPosOffset, u32& uiWeightOffset, u32& uiBoneIndexOffset, u32& uiNormOffset, u32& uiColorOffset, u32& uiTexOffset)
{
	...
	static SColor whiteVertexColor(255, 255, 255, 255);
	...
	if(uiColorOffset != UINT_MAX)
	{
		...
		for(u32 ui = 0; ui < pVertexColor->getNumVertices(); ++ui, pLockVertexColor += pVertexColor->getStride())
		{
			if(pGemData->getVertexColors())
			{
				EzMemoryCopy(&pLockVertexColor[uiColorOffset], &pGemData->getVertexColors()[ui], sizeof(SColor));
			}
			else
			{
	...
	if(uiTexOffset != UINT_MAX)
	{
		...
		for(u32 n = 0; n < pGemData->getNumUVSets(); ++n)
		{
			usStreamCount++;
			...
			for(u32 ui = 0; ui < pTexPostionStream->getNumVertices(); ++ui, pTextPosLock += pTexPostionStream->getStride())
			{
...
void EzD3DX9Driver::prePareRenderPass(EzGeometry* pGeometry)
{
	...
	//		for(u32 ui = 0; ui < pHWSkinPartition->getPartitionCount(); ++ui)
	//		{
...
//
//				for(u32 i = 0; i < pPartition->m_uiNumMatrices; ++i)
//				{
...
{
	return new EzD3DX9Texture(surface, this, m_uiTextureCreationFlags, name);
}
...
u32 EzD3DX9Driver::getMaximalDynamicLightAmount() const
{
...
{
	for (u32 i=0; i<m_uiLastSetLight+1; ++i)
...
void EzD3DX9Driver::setDynamicLight(const u32 uindex, EzLight* pLight)
{
...
bool EzD3DX9Driver::setResize(void* WindowId, s32 iWidth, s32 iHigth)
{
...
bool EzD3DX9Driver::createMultiRenderer(void* WindowID, s32 iWidth, s32 iHigth)
{
	EZASSERT(m_pMultiViewRenderer);
...
bool EzD3DX9Driver::setMeshBufferDataBinding(EzVertexData* pVertexData)
{
	...
	for (u16 unused = uiSource; unused < m_uiLastVertexSourceCount; ++unused)
	{
	...
	case EGP_VERTEX_PROGRAM:
		{
			...
			for(; !it->atEnd(); (*it)++)
			{
				u32 uiLogicalIndx = (*it)->getKey();
				const f32* pFloat = pParams->getFloatPointer((*it)->getValue().uiPhysicalIndex);
				u32 uiSlotCount = (*it)->getValue().uiCurrentSize / 4;
...
{
	EzD3DX9Driver* dx9 = new EzD3DX9Driver();
