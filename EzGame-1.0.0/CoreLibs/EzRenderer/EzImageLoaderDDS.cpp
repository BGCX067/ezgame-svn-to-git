...
bool EzImageLoaderDDS::isALoadableFileExtension(const c8* fileName) const
{
...
EzImage* EzImageLoaderDDS::loadImage(EzFile* file) const
{
	...
	if( header.size == 124 && ( header.flags & DDSD_PIXELFORMAT ) && ( header.flags & DDSD_CAPS ) )
	{
		...
		if( header.pixelFormat.flags & DDPF_RGB )
		{
			...
			{
				if( is3D )
					image = new EzImage( format, EzDimension2d< s32 >( header.width, header.height ), header.depth , data );
				else
					image = new EzImage( format, EzDimension2d< s32 >( header.width, header.height ), data );
			}
		}
