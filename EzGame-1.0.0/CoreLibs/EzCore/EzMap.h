...
//! EzMap template for associative arrays using a red-black tree
template <class KeyType, class ValueType>
class EZCORE_ENTRY EzMap : public EzMemObject
{
	//! red/black tree for EzMap
	template <class KeyTypeRB, class ValueTypeRB>
	class RBTree : public EzMemObject
	{
	public:
	...
	//! Normal Iterator
	class Iterator : public EzMemObject
	{
	public:
	...
	be the same. */
	class ParentFirstIterator : public EzMemObject
	{
	public:
	...
	because you must EzDelete the children before you EzDelete
	their parent. */
	class ParentLastIterator : public EzMemObject
	{
	public:
	...
	// myTree["Foo"] = 32;
	// If "Foo" already exists update its value else insert a EzNew element.
	// int i = myTree["Foo"]
	// If "Foo" exists return its value, else throw an exception.
	class AccessClass : public EzMemObject
	{
		// Let EzMap be the only one who can instantiate this class.
		friend class EzMap<KeyType, ValueType>;
	...
	private:
		...
		AccessClass(EzMap& tree, const KeyType& key) : Tree(tree), Key(key) {}
	...
	//! Inserts a EzNew node into the tree
	...
	bool insert(const KeyType& keyNew, const ValueType& v)
	{
		...
		Node* newNode = EzNew Node(keyNew,v);
		if (!insert(newNode))
		{
			EzDelete newNode;
			return false;
	...
	explicit EzMap(const EzMap& src);
	EzMap& operator = (const EzMap& src);
