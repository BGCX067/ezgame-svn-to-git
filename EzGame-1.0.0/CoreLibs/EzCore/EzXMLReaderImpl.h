...
//! implementation of the NeriXMLReader
template<class char_type, class superclass>
class EzXMLReaderImpl : public EzXMLReader<char_type, superclass>
{
public:

	//! Constructor
	EzXMLReaderImpl(EzFile* file)
	...
	float getAttributeValueAsFloat(const char_type* name) const
	{
		...
		EzStringc c = attr->Value.c_str();
		return EzFast_atof(c.c_str());
	}
...
private:
	...
	bool setText(char_type* start, char_type* end)
	{
		...
		EzString<char_type> s(start, (int)(end - start));
	...
	void parseComment()
	{
		...
		P -= 3;
		NodeName = EzString<char_type>(pCommentBegin+2, (int)(P - pCommentBegin-2));
		P += 3;
	}
	...
	void parseOpeningXMLElement()
	{
		...
		while(*P != L'>')
		{
			...
			else
			{
				if (*P != L'/')
				{
					...
					SAttribute attr;
					attr.Name = EzString<char_type>(attributeNameBegin, 
						(int)(attributeNameEnd - attributeNameBegin));

					EzString<char_type> s(attributeValueBegin, 
		...
		NodeName = EzString<char_type>(startName, (int)(endName - startName));
	...
	void parseClosingXMLElement()
	{
		...
		NodeName = EzString<char_type>(pBeginClose, (int)(P - pBeginClose));
		++P;
	}
	...
	bool parseCDATA()
	{
		...
		if ( cDataEnd )
			NodeName = EzString<char_type>(cDataBegin, (int)(cDataEnd - cDataBegin));
		else
			NodeName = "";
	...
	struct SAttribute
	{
		EzString<char_type> Name;
		EzString<char_type> Value;
	};
	...
	// replaces xml special characters in a EzStringc and creates a new one
	EzString<char_type> replaceSpecialCharacters(
		EzString<char_type>& origstr)
	{
	...
	bool readFile(EzFile* file)
	{
		...
		const int UTfloat_BE = 0xFFFE0000;
		const int UTfloat_LE = 0x0000FEFF;

		// check source for all utf versions and convert to target data format
		
		if (size >= 4 && data32[0] == (char32)UTfloat_BE)
		{
			// UTF-32, big endian
			SourceFormat = ETF_UTfloat_BE;
			convertTextData(data32+1, data8, (size/4)); // data32+1 because we need to skip the header
		}
		else
		if (size >= 4 && data32[0] == (char32)UTfloat_LE)
		{
			// UTF-32, little endian
			SourceFormat = ETF_UTfloat_LE;
			convertTextData(data32+1, data8, (size/4)); // data32+1 because we need to skip the header
		}
		else
	...
	//! generates a EzList with xml special characters
	void createSpecialCharacterList()
	{
		// EzList of strings containing special symbols, 
		// the first character is the special character,
		// the following is the symbol EzStringc without trailing &.
	...
	EzString<char_type> NodeName;    // name of the node currently in
	EzString<char_type> EmptyString; // empty EzStringc to be returned by getSafe() methods

	bool IsEmptyElement;       // is the currently parsed node empty?

	EzArray< EzString<char_type> > SpecialCharacters; // see createSpecialCharacterList()

	EzArray<SAttribute> Attributes; // attributes of current element
