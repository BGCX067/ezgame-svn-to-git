...
public:
	...
	//draw funtion start
	/////////////////
	virtual void draw3DLine(const EzPoint3& start, const EzPoint3& end, EzColor color);
	//draw funtion end
	//////////////////

	bool initDriver(const EzDimension2d& screenSize,	HWND hWnd, unsigned int uiBits, bool bFullScreen, bool bPureSoftware,
	...
	bool setResize(const EzDimension2d& size, void* WindowId);

	virtual void setTargetWindow(void* WindowID);
	virtual bool setResize(void* WindowId, int iWidth, int iHigth);
	virtual bool createMultiRenderer(void* WindowID, int iWidth, int iHigth);
	...
	virtual unsigned int getMaximalDynamicLightAmount() const;


	bool isHardwareSkinning();

	virtual bool setTexture(unsigned int eMap, EzTexture* pSource);
...
protected:
	...
	virtual void setDynamicLight(const unsigned int uindex, EzLight* pLight);
	virtual void clearDynamicLights();
	virtual void drawVertexPrimitive(EPRIMITIVE_TYPE eType, unsigned int vertexStart, unsigned int vertexCount, unsigned int indexStart);
...
private:
	...
	unsigned int								m_uiVertexBlendMatricesCount;
	...
	unsigned int							m_uiLastSetLight;
	unsigned short							m_uiLastVertexSourceCount;

	
	HINSTANCE					m_hD3DLibrary;
	EzDimension2d				m_CurrentDepthBufferSize;
