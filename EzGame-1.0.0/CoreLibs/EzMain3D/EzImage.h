...
public:
	...
	//! returns a pixel
	virtual EzColor getPixel(unsigned int x, unsigned int y) const;

	//! returns a pixel from a volume texture
	EzColor getPixel(unsigned int x, unsigned int y, unsigned int z) const;

	//! sets a pixel
	virtual void setPixel(unsigned int x, unsigned int y, const EzColor &color );
	...
	virtual void copyToScaling(void* target, int width, int height, ECOLOR_FORMAT format, unsigned int pitch=0);

	//! copies this surface into another, scaling it to fit.
	virtual void copyToScaling(EzImage* target);

	//! copies this surface into another
	virtual void copyTo(EzImage* target, const EzDimension2d& pos=EzDimension2d(0,0));

	//! copies this surface into another
	virtual void copyTo(EzImage* target, const EzDimension2d& pos, const EzRect& sourceRect, const EzRect* clipRect=0);

	//! copies this surface into another, using the alpha mask, an cliprect and a color to add with
	virtual void copyToWithAlpha(EzImage* target, const EzDimension2d& pos,
			const EzRect& sourceRect, const EzColor &color,
...
private:
	...
	inline EzColor getPixelBox ( int x, int y, int fx, int fy, int bias ) const;
